sstring = """Game 1: 3 blue, 7 green, 10 red; 4 green, 4 red; 1 green, 7 blue, 5 red; 8 blue, 10 red; 7 blue, 19 red, 1 green
...
def get_min_count(color_string: str):
    numbers_of_given_color = []
    line_elements = line.split()

    for element_index in range(len(line_elements)):
        if color_string in line_elements[element_index]:
            numbers_of_given_color.append(int(line_elements[element_index - 1]))
    return max(numbers_of_given_color)


for line in sstring.splitlines():
    game_min_reds_no = get_min_count("red")
    game_min_greens_no = get_min_count("green")
    game_min_blues_no = get_min_count("blue")

    power = game_min_reds_no * game_min_greens_no * game_min_blues_no
    sum_of_powers += power
