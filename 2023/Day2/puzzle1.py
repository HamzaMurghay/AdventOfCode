...
impossible = [str(num) for num in range(15, 20)]
impossible.append("13 red")
impossible.append("14 red")
impossible.append("14 green")

game_nos = [num for num in range(1, 101)]
count = -1
for line in sstring.splitlines():
    count += 1
    multiple_impossibles_flag = 0
    for imp in impossible:
        if imp in line.split(":")[1] and multiple_impossibles_flag == 0:
            game_nos.remove(count+1)
            multiple_impossibles_flag += 1
